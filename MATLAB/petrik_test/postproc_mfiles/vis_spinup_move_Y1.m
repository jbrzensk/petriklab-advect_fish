...
%%
behave = {'enc','ingest','mort','nu','prey'};

for b=1:length(behave)
    
    mod = ['Spinup1988_move_',behave{b},'_v21_dt6h_newdt_All_fish03_Y1'];
    ...
    numnan(1,1) = sum(isnan(sf_mean1));
    numnan(1,2) = sum(isnan(sf_mean2));
    numnan(1,3) = sum(isnan(sf_mean3));
    numnan(1,4) = sum(isnan(sf_mean));
    numnan(2,1) = sum(isnan(sp_mean1));
    numnan(2,2) = sum(isnan(sp_mean2));
    numnan(2,3) = sum(isnan(sp_mean3));
    numnan(2,4) = sum(isnan(sp_mean));
    numnan(3,1) = sum(isnan(sd_mean1));
    numnan(3,2) = sum(isnan(sd_mean2));
    numnan(3,3) = sum(isnan(sd_mean3));
    numnan(3,4) = sum(isnan(sd_mean));
    numnan(4,1) = sum(isnan(mf_mean1));
    numnan(4,2) = sum(isnan(mf_mean2));
    numnan(4,3) = sum(isnan(mf_mean3));
    numnan(4,4) = sum(isnan(mf_mean));
    numnan(5,1) = sum(isnan(mp_mean1));
    numnan(5,2) = sum(isnan(mp_mean2));
    numnan(5,3) = sum(isnan(mp_mean3));
    numnan(5,4) = sum(isnan(mp_mean));
    numnan(6,1) = sum(isnan(md_mean1));
    numnan(6,2) = sum(isnan(md_mean2));
    numnan(6,3) = sum(isnan(md_mean3));
    numnan(6,4) = sum(isnan(md_mean));
    numnan(7,1) = sum(isnan(lp_mean1));
    numnan(7,2) = sum(isnan(lp_mean2));
    numnan(7,3) = sum(isnan(lp_mean3));
    numnan(7,4) = sum(isnan(lp_mean));
    numnan(8,1) = sum(isnan(ld_mean1));
    numnan(8,2) = sum(isnan(ld_mean2));
    numnan(8,3) = sum(isnan(ld_mean3));
    numnan(8,4) = sum(isnan(ld_mean));

    save([fpath mod '.mat'],'numnan','-append')
