%%%% File naming system
function [fname,simname,sname] = sub_fname_spin_move_core(param,odir,exper)
...
if (frate==0)
    ...
    sname = [outdir,'Last_mo_Spinup1988_no_move_pristine'];
elseif (param.Jsel~=0.1)
    fname = [outdir,exper, '_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
    sname = [outdir,'Last_mo_Spinup1988_no_move_', sel,'_fish',tfish(2:end),'_Juve',tJ(2:end)];
elseif (param.MFsel~=param.LPsel)
    fname = [outdir,exper, '_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
    sname = [outdir,'Last_mo_Spinup1988_no_move_fish_F',tF(2:end),'_P',tP(2:end),'_D',tD(2:end)];
else
    fname = [outdir,exper, '_', sel,'_fish',tfish(2:end)];
    sname = [outdir,'Last_mo_Spinup1988_no_move_', sel,'_fish',tfish(2:end)];
end
