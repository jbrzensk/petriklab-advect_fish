...
function [Sf,Sp,Sd,Mf,Mp,Md,Lp,Ld,BENT,ENVR] = sub_futbio_move_prey_K(DY,ESM,GRD,Sf,Sp,Sd,Mf,Mp,Md,Lp,Ld,BENT,param,neighbor)
...
% Some other plots
% plotNineFish( 18, 'Pre bio', Sf.bio, Sp.bio, Sd.bio, Mf.bio,...
%                                  Mp.bio, Md.bio, Lp.bio, Ld.bio,...
%                                  GRD, param);
% 
% plotNineFish( 19, 'Pre con zm', Sf.con_zm, Sp.con_zm, Sd.con_zm, Mf.con_zm,...
%                                  Mp.con_zm, Md.con_zm, Lp.con_zm, Ld.con_zm,...
%                                  GRD, param);
...
% Some other plots
% plotNineFish( 20, 'Cons Rates', Sf.I, Sp.I, Sd.I, Mf.I, Mp.I, Md.I, Lp.I, Ld.I, GRD, param);
% plotNineFish( 21, 'Post con zm', Sf.con_zm, Sp.con_zm, Sd.con_zm, Mf.con_zm,...
%                                  Mp.con_zm, Md.con_zm, Lp.con_zm, Ld.con_zm,...
%                                  GRD, param);
...
diff = sum( bioSf(:) - old_bioSf(:), 'omitnan');
assert( diff < diftol, 'Sf Diff too large, diff=%4.5f', diff);
diff = sum( bioSp(:) - old_bioSp(:), 'omitnan');
assert( diff < diftol, 'Sp Diff too large, diff=%4.5f', diff);
diff = sum( bioSd(:) - old_bioSd(:), 'omitnan');
assert( diff < diftol, 'Sd Diff too large, diff=%4.5f', diff);
diff = sum( bioMf(:) - old_bioMf(:), 'omitnan');
assert( diff < diftol, 'Mf Diff too large, diff=%4.5f', diff);
diff = sum( bioMp(:) - old_bioMp(:), 'omitnan');
assert( diff < diftol, 'Mp Diff too large,, diff=%4.5f', diff);
diff = sum( bioMd(:) - old_bioMd(:), 'omitnan');
assert( diff < diftol, 'Md Diff too large, diff=%4.5f', diff);
diff = sum( bioLp(:) - old_bioLp(:), 'omitnan');
assert( diff < diftol, 'Lp Diff too large, diff=%4.5f', diff);
...
% nexttile;pcolor(bioSd'); shading interp;title('Sdbio');colorbar;
% nexttile; pcolor(preySd'); shading interp;title('Sdprey');colorbar;
% nexttile; pcolor(bioSp'); shading interp;title('Spbio');colorbar;
% nexttile; pcolor(preySp'); shading interp;title('Spprey');colorbar;
% drawnow;
