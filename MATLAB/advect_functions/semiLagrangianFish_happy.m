function [Flux] = semiLagrangianFish_happy(conc_matrix, Flux, idx, dir, current, dt, dx_m, dy_m, neighbors, grid_mask, area)
...
% -------------------------------------------------------------------------
    ...
    % dir is where food is, prevents going onto land, even if current is
    % pushing that way.
    speeds = dir .* current;
    %
